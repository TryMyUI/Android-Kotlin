...
class ParserMp4 {
    ...
    fun append(
        ...
        try {
            Log.e(
                TAG,
                "new FileDataSourceImpl(firstfile) " + FileDataSourceImpl(
                    firstFile
                )
            )
            Log.e(
                TAG,
                "new FileDataSourceImpl(secondFile) " + FileDataSourceImpl(
                    secondFile
                )
            )
            val movieA =
                MovieCreator.build(FileDataSourceImpl(secondFile))
            val movieB =
                MovieCreator.build(FileDataSourceImpl(firstFile))
            val finalMovie =
                Movie()
            val movieOneTracks =
                movieA.tracks
