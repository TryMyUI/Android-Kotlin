...
class SpecialQualificationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tv_special_qual.text = Html.fromHtml(availableTestModel.specialQalification)

        btn_spec_cont.setOnClickListener { onContinueClick() }

        rg_special_qual.setOnCheckedChangeListener { group, checkedId ->
            btn_spec_cont.setBackgroundResource(R.drawable.trymyuitester_btn_bg)
            btn_spec_cont.setTextColor(resources.getColor(R.color.white))
            btn_spec_cont.isClickable = true
        }

        ll_three_dots.setOnClickListener { v -> showMenuPopUp(v) }

        val scrollView = findViewById(R.id.nsw) as ScrollView
        val childHeight = (findViewById(R.id.ll_scroll_content) as LinearLayout).height
        val isScrollable =
            scrollView.height < childHeight + scrollView.paddingTop + scrollView.paddingBottom

        Log.e(TAG,"isScrollable");


    }

    private fun onContinueClick(){
        if (rg_special_qual.checkedRadioButtonId == R.id.rb_yes)
        {
            ManageFlowBeforeRecording(availableTestModel, this@SpecialQualificationActivity)
                .moveToWhichActivity(1)
            finish()
        }
        else
        {
    ...
    private fun showMenuPopUp(view1: View) {
        ...
        popupWindow = PopupWindow(view1, density.toInt() * 240, WindowManager.LayoutParams.WRAP_CONTENT, true)
        val view: View
        val layoutInflater =
        ...
        tv_menu_reportissue.setOnClickListener {
            ...
            val intentFeedback = Intent(
                this@SpecialQualificationActivity,
    ...
    private fun showConfirmation() {
        ...
        dialogBuilder.setPositiveButton(
            ...
            if (SharedPrefHelper(this@SpecialQualificationActivity).getGuestTester()) {
                val intent =
                    Intent(this@SpecialQualificationActivity, WelcomeActivity::class.java)
                startActivity(intent)
                finish()
            } else {
                val intent =
                    Intent(this@SpecialQualificationActivity, TabActivity::class.java)
                startActivity(intent)
                finish()
            }
        }
        show_confirmation_dialog = dialogBuilder.create()
        val dialogWindow: Window = show_confirmation_dialog!!.window
