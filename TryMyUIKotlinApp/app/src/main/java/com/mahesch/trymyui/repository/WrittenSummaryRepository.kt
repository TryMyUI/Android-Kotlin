...
class WrittenSummaryRepository (application: Application) {

    private var apiInterface: ApiService.ApiInterface = RetrofitInstance.getPostApiService()
    private var TAG = WrittenSummaryRepository::class.java.simpleName.toUpperCase()
    ...
    init {
     ...
     fun submitWrittenSummary(writtenSummaryJsonRequest: JsonObject) : MutableLiveData<CommonModel>{

        var call = apiInterface.postWrittenSummaryAnswers(writtenSummaryJsonRequest)
